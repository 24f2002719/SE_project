    ...
    <div class="space-y-gutter">
            ...
            <div class="space-y-md">
              ...
              >
                ...
                <div class="space-y-sm text-xs text-on-surface-variant">
                  ...
                  <p class="flex items-center gap-xs"><span class="material-symbols-outlined text-sm">rule</span> Req. Attendance: {{ e.attendanceRequired !== false ? (e.minAttendance !== undefined ? e.minAttendance : 75) + '%' : 'None' }}</p>
      ...
      <div v-if="selectedRegEvent" class="fixed inset-0 bg-primary/40 z-50 flex items-center justify-center p-md">
        <form @submit.prevent="submitRegistration" class="bg-white border border-outline-variant rounded-xl w-full max-w-md shadow-lg flex flex-col p-md space-y-md" @click.stop>
          ...
          <div class="space-y-1 text-xs">
            <label class="block font-semibold text-on-surface-variant">Roll Number / Student ID</label>
...
const submitRegistration = () => {
  ...
  if (ev) {
    const isRequired = ev.attendanceRequired !== false
    const threshold = isRequired ? (ev.minAttendance !== undefined ? ev.minAttendance : 75) : 0
    ev.roster.push({
      name: regForm.name,
      email: regForm.email,
